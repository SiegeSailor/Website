...
- `.github/` — GitHub-related files: workflows (`static-production.yml` deploys the static site; `release.yml` runs semantic-release, which versions the project per `release.config.mjs` and attaches resume documents to every release), Copilot instruction files in `instructions/`.
