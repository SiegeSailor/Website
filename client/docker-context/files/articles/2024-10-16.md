---
title: Building Embedded WPE WebKit on a Linux Desktop
category: Embedded
status: Ready
technologies: ["Linux", "WebKit", "Wayland"]
---

I ran into this challenge while trying to set up WPE WebKit on my Linux machine for my work at [CooperSurgical](https://www.coopersurgical.com/). WPE is typically used on embedded systems, but I couldn’t get the actual board running at home while working remotely. To work around that, I decided to install it on my laptop so I could simulate the browser environment and test my web application.

Most of the documentation out there focuses on building WPE WebKit for embedded platforms. This guide is for developers like me who want to run it on a regular X11 desktop for debugging purposes. Inspired by [Hikiko's Post for WPE](https://eleni.mutantstargoat.com/hikiko/wpe/), I put together a simpler installation process based on what worked for me.

<!-- description -->

> Follow [WPE Webkit Wiki](https://trac.webkit.org/wiki/WPE) when developing on an embedded platform with Yocto or OpenEmbedded.

Hardware matters. I did this with [Darter Pro from System76](https://system76.com/laptops/darter):
...
WebKit is now ready to use. However, in order to run WebKit under an X11 system, we need to run `cog` in a **Weston** environment:
...
The following photo shows a running Webkit on a System76 Darter Pro. The left terminal runs `weston` while the right terminal runs the `cog` browser displaying [https://www.google.com](https://www.google.com):

![Weston-Cog-Google](/images/2024-10-16/Weston-Cog-Google.png)
