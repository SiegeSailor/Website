...
- **Share a Step through `.github/actions/`**: Every `run` there needs its own `shell: bash`, `Checkout` is the one step that cannot move there, and `setup-workspace` owns Node.js and `npm ci` for all 4 workflows
