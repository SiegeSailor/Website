...
A note of a great example of building the road while driving. When I was working at [CooperSurgical](https://www.coopersurgical.com/), I received a request from another team, requesting a .NET support for integration with our product. The integration included network discovering, message queueing, payload parsing, and stable versioning management. I had no experience on .NET development, but I did know what technologies the package needed to work with our product. I vibe-coded the library at the beginning, and then turned out picked up pretty fast. Overall, it wasn't too difficult to develop a .NET package, as it is quite similar to C++ and Java.

I ended up building a NuGet package using .NET SDK 8.0, and built a client application for demonstration using .NET SDK 8.0 Web and the NuGet package I built. To have my client application integrate the package, I developed a pipeline to distribute the package using GitLab CI/CD and GitLab Package Registry, following [Git Flow](/blog/2024-08-22).
...
- [VS Code with .NET Extensions](https://code.visualstudio.com/docs/languages/dotnet): Edit and manage .NET code
...
You can create your `tag-package.sh` script to map `CI_COMMIT_TAG` according the code flow and branches. See [Long-Term Support](/blog/2024-08-19) and [Git Flow](/blog/2024-08-22).
...
## Installing Package in a Consumer Project
