...
`articles/` is plainer: `../website/helpers/server/article.ts` reads the folder from disk, and the front matter in each file drives the post list, the feed, and the article page. Images live with the website, in `../website/public/images/<YYYY-MM-DD>/`, because Next.js serves them.
