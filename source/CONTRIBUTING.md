...
- **`settings/content.ts`**: Every word the site renders that is not an article, authored as plain typed constants — `getSite()` for the header, footer, and every `<head>`, `getHome()` for the home hero, and `getAbout()` for `/about`. A page that needs a new value adds it to the right accessor
